...
import { Link, Share2, Ticket, Calendar, Users, DollarSign, Trophy, MessageCircle, AlertCircle, Clock, CreditCard } from 'lucide-react';
...
import { Campaign, Promotion, Prize } from '../types/campaign';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {campaign.prizes && campaign.prizes.length > 0 && (
              <div className={`${themeClasses.cardBg} rounded-2xl shadow-xl p-6 border ${themeClasses.border}`}>
                ...
                <div className="space-y-3">
                  {campaign.prizes.map((prize: Prize, index: number) => (
          ...
          <div className="lg:col-span-1 space-y-8">
            {/* Share Section */}
            <div className={`${themeClasses.cardBg} rounded-2xl shadow-xl p-6 border ${themeClasses.border}`}>
              <h2 className="text-xl font-bold mb-4 text-center">Compartilhar</h2>
            ...
            <div className={`${themeClasses.cardBg} rounded-2xl shadow-xl p-6 border ${themeClasses.border}`}>
              <h3 className={`text-sm font-medium ${themeClasses.textSecondary} uppercase mb-4 text-center`}>MÉTODO DE PAGAMENTO</h3>
              <div className="flex items-center justify-center space-x-3 bg-green-500 rounded-lg p-4">
                <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center">
                  <span className="text-green-500 font-bold text-sm">₽</span>
                </div>
                <p className="text-white text-lg font-semibold">PIX</p>
              </div>
            </div>

            {/* Draw Method Section */}
            <div className={`${themeClasses.cardBg} rounded-2xl shadow-xl p-6 border ${themeClasses.border}`}>
              <h3 className={`text-sm font-medium ${themeClasses.textSecondary} uppercase mb-4 text-center`}>SORTEIO</h3>
              <div className="flex items-center justify-center space-x-3 bg-blue-500 rounded-lg p-4">
                <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center">
                  <Calendar className="h-5 w-5 text-blue-500" />
                </div>
                <p className="text-white text-lg font-semibold">{campaign.draw_method}</p>
              </div>
            </div>

            {/* NEW: Draw Date Section - Only show if draw_date exists */}
            {campaign.draw_date && (
              <div className={`${themeClasses.cardBg} rounded-2xl shadow-xl p-6 border ${themeClasses.border}`}>
                <h3 className={`text-sm font-medium ${themeClasses.textSecondary} uppercase mb-4 text-center`}>DATA</h3>
                <div className="text-center">
                  <p className={`text-lg font-bold ${themeClasses.text}`}>
                    Data: {formatDrawDate(campaign.draw_date)}
                  </p>
                </div>
              </div>
            )}
            ...
            {/* Campaign Stats */}
            <div className={`${themeClasses.cardBg} rounded-2xl shadow-xl p-6 border ${themeClasses.border}`}>
              <h2 className="text-xl font-bold mb-4 text-center">Estatísticas</h2>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <Users className="h-5 w-5 text-blue-500" />
                    <span className={themeClasses.textSecondary}>Vendidas</span>
                  </div>
                  <span className="font-bold">{campaign.sold_tickets}</span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <Ticket className="h-5 w-5 text-green-500" />
                    <span className={themeClasses.textSecondary}>Disponíveis</span>
                  </div>
                  <span className="font-bold">{campaign.total_tickets - campaign.sold_tickets}</span>
                </div>
            ...
            {/* Campaign Owner Info */}
            <div className={`${themeClasses.cardBg} rounded-2xl shadow-xl p-6 border ${themeClasses.border}`}>
              <h2 className="text-xl font-bold mb-4 text-center">Organizado por</h2>
