...
interface CampaignFormData {
    ...
    ticketPrice: number;
    totalTickets: number;
    acceptCampaignTerms: boolean;
    // ... outros campos necessários
}

// Componente Wrapper para o Checkbox Animado, para evitar repetição no JSX
interface CheckboxWrapperProps {
    id: string;
    checked: boolean;
    onChange: (checked: boolean) => void;
    label: React.ReactNode;
    isTerms?: boolean; // Para aplicar estilos diferentes na caixa de Termos
}

const CheckboxWrapper: React.FC<CheckboxWrapperProps> = ({ id, checked, onChange, label, isTerms = false }) => (
    <motion.div 
        className={`p-4 rounded-xl border-2 transition-colors duration-200 ${
            isTerms 
                ? 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 shadow-md'
                : (checked 
        ...
        <label htmlFor={id} className="flex items-start gap-3 cursor-pointer group">
            ...
            >
                <input
                    ...
                    required={id === 'acceptCampaignTerms'}
                />
                <motion.div
                    ...
                    style={checked ? { borderColor: 'transparent', backgroundColor: 'transparent' } : {} as React.CSSProperties}
                >
                    <AnimatePresence>
                        {checked && (
                            <motion.div
            ...
            <span className={`text-base font-medium leading-relaxed cursor-pointer ${
                isTerms ? 'text-gray-700 dark:text-gray-300' : 'text-gray-900 dark:text-white'
            }`}>
                {label}
            </span>
        </label>
      ...
      <main className="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 sm:p-10">
        ...
        <form onSubmit={handleSaveAndContinue} className="space-y-8">
            ...
            <div className="space-y-6 pt-6 border-t dark:border-gray-700">
                ...
                {/* Configurações de Checkbox/Switch - ATUALIZADO COM CHECKBOX ANIMADO */}
                <div className="space-y-4">
                    ...
                    <CheckboxWrapper
                        ...
                        onChange={(checked) => handleCheckboxChange('requireEmail', checked)}
                        label="Exigir E-mail no Cadastro"
                    />
                    ...
                    <CheckboxWrapper
                        ...
                        onChange={(checked) => handleCheckboxChange('showRanking', checked)}
            ...
            {/* SEÇÃO DO CHECKBOX ANIMADO (TERMOS DE CAMPANHA) */}
            <motion.div 
            ...
            >
              ...
              <CheckboxWrapper
                ...
                onChange={(checked) => handleCheckboxChange('acceptCampaignTerms', checked)}
                isTerms={true}
                label={(
                    <>
