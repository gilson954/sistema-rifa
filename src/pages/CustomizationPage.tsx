...
import { Upload, Plus, ArrowRight, X, Loader2, Trash2, ExternalLink, CheckCircle, AlertCircle, Clock, Sparkles, Palette, Eye, Lightbulb } from 'lucide-react';
...
const CustomizationPage = () => {
  ...
  const confirmRemoveLogo = async () => {
    ...
    setUploadingLogo(true);
    try {
      // Logic to determine the file path from the full URL
      const urlParts = currentLogoUrl.split('/');
      // The last part of the URL is the filename, but we need the path relative to the bucket root, which is typically user_id/filename
      // Assuming the bucket structure is 'logos/user_id/filename'
      ...
      if (deleteError) {
        ...
        // Do not throw, continue to update profile even if storage delete fails
      }
      ...
      <AnimatePresence>
        {showDomainModal && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={() => { if (!saving) setShowDomainModal(false); }} // Fecha ao clicar no backdrop, a menos que esteja salvando
          >
            <motion.div
            ...
            >
              <div className="flex items-center justify-between mb-4 sm:mb-6">
                ...
                <motion.button
                  onClick={() => { if (!saving) setShowDomainModal(false); }}
                  disabled={saving}
                  className="p-1 sm:p-2 rounded-full text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors disabled:opacity-50"
                  whileHover={{ scale: 1.1 }}
                  whileTap={{ scale: 0.9 }}
                >
              ...
              <div className="space-y-4 sm:space-y-6">
                <input
                  ...
                  disabled={saving}
                  className="w-full px-4 sm:px-5 py-3 sm:py-3.5 border border-gray-300 dark:border-gray-600 rounded-lg sm:rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-purple-500 focus:border-purple-500 transition-colors duration-200 text-sm sm:text-base disabled:opacity-70"
                />
