...
// Simulação de hooks e APIs (substitua pelos seus reais)
const useAuth = () => ({ user: { id: '123', email: 'user@example.com' } });
...
const SuggestionsAPI = {
  ...
  createSuggestion: async (formData: any, userId?: string) => {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      setFormData(prev => ({
        ...
        user_email: user?.email || 'joao@example.com'
      }));
      setLoading(false);
    };
    loadUserData();
